export const DOWNLOADS_MAP: Record<string, string> = {
  "ecac-001": "Instalador_ECAC.exe",
  "fgts-001": "Instalador_FGTS.exe",
  "whats-001": "Instalador_EnvioWhatsApp.exe",
  "email-001": "Instalador_EnvioEmail.exe",
  "central-documentos-001": "Instalador - CentralDocumentos.exe",
  "dre-001": "Instalador_DRE.exe",
  "funcionarios-001": "PalSysRunnerPhoenix-Setup.exe",
  "pdf-monitor-001": "Instalador_PalSysPDFMonitor.exe",
  "consolida-impostos-001": "Instalador_PalSys_Conciliador_de_Impostos.exe",
  "consolidador-folhas-001": "Instalador_PalSys_Consolidador_de_Folha.exe",
  "dctfweb-integra-001": "Instalador_PalSys_Consulta_DCTFWeb.exe",
}
