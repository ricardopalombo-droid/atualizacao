...
def preencher_campo_dependente(valor):
    ...
    pyautogui.press("backspace")
    dormir_controlado(0.08)
    escrever(texto, pausa_antes=0.05, pausa_depois=0.15)
    pyautogui.press("tab")
    dormir_controlado(0.25)
...
def selecionar_uf_em_lista(sigla_uf: str, confirmar=True):
    texto = limpar_texto(sigla_uf).strip().upper()
    if not texto:
        if confirmar:
            pyautogui.press("tab")
            dormir_controlado(0.25)
...
def selecionar_sp():
    selecionar_uf_em_lista("SP", confirmar=True)
...
def selecionar_sigla_uf(valor):
    selecionar_uf_em_lista(valor, confirmar=True)


def selecionar_sigla_uf_sem_enter(valor):
    selecionar_uf_em_lista(valor, confirmar=False)
