...
class HotkeyCalibrationUI:
    def __init__(self, notes_file: Path):
        ...
        self.root = tk.Tk()
        self.root.title("Calibrador Phoenix por Atalhos")
        ...
        self.status_var = tk.StringVar(
            value="Atalhos: F6 executa | F7 certo | F8 errado | F9 pular | ESC encerrar"
        )
    ...
    def _build_layout(self):
        ...
        tk.Label(wrapper, textvariable=self.title_var, font=("Segoe UI", 14, "bold"), wraplength=520, justify="left").pack(anchor="w")
        tk.Label(wrapper, textvariable=self.field_var, wraplength=520, justify="left").pack(anchor="w", pady=(8, 0))
        tk.Label(wrapper, textvariable=self.legacy_var, wraplength=520, justify="left").pack(anchor="w", pady=(4, 0))
        tk.Label(wrapper, textvariable=self.value_var, wraplength=520, justify="left").pack(anchor="w", pady=(4, 8))

        tk.Label(wrapper, text="Atalhos", font=("Segoe UI", 10, "bold")).pack(anchor="w")
        tk.Label(
            wrapper,
            text="F6 = Executar etapa | F7 = Ficou certo | F8 = Ficou errado | F9 = Pular | ESC = Encerrar",
            wraplength=520,
            justify="left",
        ...
        self.expected_source_entry = tk.Entry(form, width=62)
        ...
        self.phoenix_instruction_entry = tk.Entry(form, width=62)
        ...
        self.notes_text = scrolledtext.ScrolledText(form, width=60, height=6, wrap="word")
        self.notes_text.grid(row=5, column=0, sticky="we")

        tk.Button(
            form,
            text="Salvar ajuste do item errado",
            command=self._save_wrong_note,
        ).grid(row=6, column=0, sticky="w", pady=(8, 0))

        form.columnconfigure(0, weight=1)

        tk.Label(wrapper, textvariable=self.status_var, fg="#92400e", wraplength=520, justify="left").pack(anchor="w", pady=(10, 8))

        tk.Label(wrapper, text="Notas registradas nesta sessao", font=("Segoe UI", 10, "bold")).pack(anchor="w")
        self.log_text = scrolledtext.ScrolledText(wrapper, width=62, height=12, wrap="word")
    ...
    def show_step(self, meta: dict):
        ...
        self.status_var.set("Com o Phoenix em foco, pressione F6 para executar esta etapa.")
    ...
    def mark_after_execute(self):
        self.status_var.set("Veja o Phoenix. F7 se ficou certo | F8 se ficou errado | F9 para pular a proxima.")

    def prepare_wrong(self):
        self.pending_wrong = True
        self.root.deiconify()
        self.root.lift()
        self.root.focus_force()
        self.status_var.set("Preencha os ajustes abaixo e clique em 'Salvar ajuste do item errado'.")

    def _save_wrong_note(self):
        if not self.pending_wrong or not self.current_meta:
            messagebox.showwarning("Calibracao Phoenix", "Nenhum item errado aguardando anotacao.")
            return
        ...
        self.pending_wrong = False
        self.status_var.set("Ajuste salvo. Volte ao Phoenix e pressione F6 para seguir.")
...
class HotkeyCoordinator:
    def __init__(self, ui: HotkeyCalibrationUI, expected_steps: list[dict]):
        self.ui = ui
    ...
    def wait_post_command(self):
        ...
        while True:
            ...
            if command == "ok":
                return
            if command == "wrong":
                self.ui.result_queue.put({"type": "wrong"})
                return


def attach_calibration_wrappers(legacy, coordinator: HotkeyCoordinator):
    helper_names = [
        "escrever_e_tab",
        "escrever",
        "limpar_e_digitar",
...
def worker(args, ui: HotkeyCalibrationUI):
    try:
        ...
        coordinator = HotkeyCoordinator(ui, build_expected_steps(payload))
...
def main():
    parser = argparse.ArgumentParser(
        description="Executa o robô legado do Phoenix em modo de calibracao com atalhos globais."
    )
    ...
    ui = HotkeyCalibrationUI(Path(args.notes_file))
